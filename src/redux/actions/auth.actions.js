...
export const playerRegister = form =>{
    return async(dispatch) =>{
        dispatch({type: AUTH_PLAYER_REGISTER});

        const registerRequest = await fetch('http://localhost:4000/player/register', {
            method: 'POST',
            headers: {
                Accept: "application/json",
                "Content-type": "application/json",
        ...
        const result = await registerRequest.json();
        if(registerRequest.ok){
            dispatch({type: AUTH_PLAYER_REGISTER_OK, payload: result})
        }else{
            dispatch({type: AUTH_PLAYER_REGISTER_ERROR, payload: result.message})
        };
    };
};

export const playerLogin = form =>{
    return async(dispatch) =>{
        dispatch({type: AUTH_PLAYER_LOGIN});

        const loginRequest = await fetch('http://localhost:4000/player/login',{
            method: 'POST',
            headers: {
                Accept: "application/json",
                "Content-type": "application/json",
        ...
        if(loginRequest.ok){
            dispatch({type: AUTH_PLAYER_LOGIN_OK, payload: result})
        }else{
            dispatch({type: AUTH_PLAYER_LOGIN_ERROR, payload: result.message})
        };
    };
};
...
export const coachRegister = form =>{
    return async(dispatch) =>{
        dispatch({type: AUTH_COACH_REGISTER});

        const registerRequest = await fetch('http://localhost:4000/coach/register', {
            method: 'POST',
            headers: {
                Accept: "application/json",
                "Content-type": "application/json",
        ...
        if(registerRequest.ok){
            dispatch({type: AUTH_COACH_REGISTER_OK, payload: result})
        }else{
            dispatch({type: AUTH_COACH_REGISTER_ERROR, payload: result.message})
        };
    };
};

export const coachLogin = form =>{
    return async(dispatch) =>{
        dispatch({type: AUTH_COACH_LOGIN});

        const registerRequest = await fetch('http://localhost:4000/coach/login', {
            method: 'POST',
            headers: {
                Accept: "application/json",
                "Content-type": "application/json",
        ...
        if(registerRequest.ok){
            dispatch({type: AUTH_COACH_LOGIN_OK, payload: result})
        }else{
            dispatch({type: AUTH_COACH_LOGIN_ERROR, payload: result.message})
        };
    };
};
